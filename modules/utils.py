...
def show_data(msg:Message, username:User):
    ...
    for i in datos_usuarios[username]:
        ...
        indice:int = datos_usuarios[username].index(i)
        
        if media_value == 'document':
            total_size += i.document.file_size
            texto += f"**\n📄 /rm_{indice} `{sizeof(i.document.file_size)}` - {i.document.file_name}**"

        if media_value == 'photo':
            total_size += i.photo.file_size
            texto += f"**\n📷 /rm_{indice} `{sizeof(i.photo.file_size)}` - Ancho: `{i.photo.width}` / Alto: `{i.photo.height}`**"

        if media_value == 'sticker':
            total_size += i.sticker.file_size
            texto += f"**\n🧩 /rm_{indice} `{sizeof(i.sticker.file_size)}` - Sticker [{i.sticker.emoji}]**"
        
        if media_value == 'audio':
            total_size += i.audio.file_size
            texto += f"**\n🎧 /rm_{indice} `{sizeof(i.audio.file_size)}` - {i.audio.file_name}**"
        
        if media_value == 'video':
            total_size += i.video.file_size
            texto += f"**\n🎬 /rm_{indice} `{sizeof(i.video.file_size)}` - {i.video.file_name}**"
